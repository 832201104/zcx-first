  ...
  <view class="page">
    ...
    <view class="form-card">
      <form @submit="save">
        ...
        <button class="form-item btn-submit" @click="save()">
          修&nbsp;&nbsp;改
        </button>
        <button class="form-item btn-submit" @click="remove(contactId)">
          删&nbsp;&nbsp;除
        </button>
      </form>
    </view>
  ...
  methods: {
    ...
    async save() {
      try {
        ...
        uni.request({
          url: `http://127.0.0.1:8001/edit?id=${this.contactId}&name=${this.contact.name}&tel=${this.contact.tel}&remark=${this.contact.remark}&birthday=${this.contact.birthday}`, // 请求的URL
          methodt: "GET", // 请求方式
          success: (res) => {
